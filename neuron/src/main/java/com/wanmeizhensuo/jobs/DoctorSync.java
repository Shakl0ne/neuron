...
public class DoctorSync {
    @Inject
    TopicConfiguration topConfig;

    @Inject
    StreamsConfiguration stmConfig;
    ...
    public void ApiDoctorSync(@Observes StartupEvent startupEvent) {
        SyncVerticle.flow("doctor_sync")
                .bootstrapServers(stmConfig.bootstrapServers)
                .topic(topConfig.doctorSync)
                .groupId(topConfig.doctorGroup)
                .consumers(1)
